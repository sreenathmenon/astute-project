...
from openstack_dashboard.api.astute import get_admin_ksclient, \
                                           get_cinder_client, \
                                           get_neutron_client, \
                                           get_nova_client, \
                                           is_m1_user_admin, \
                                           tenant_quota_update_nova, \
                                           tenant_quota_get_nova, \
                                           server_list_nova, \
                                           flavor_list_nova, \
                                           flavor_get_nova, \
                                           tenant_absolute_limits_nova, \
                                           tenant_quota_update_cinder, \
                                           tenant_quota_get_cinder, \
                                           get_volume_snapshot_list, \
                                           get_volume_list, \
                                           tenant_absolute_limits_cinder, \
                                           get_tenant_quota_neutron, \
                                           network_list_neutron, \
                                           subnet_list_neutron, \
                                           router_list_neutron
...
def _get_tenant_compute_usages(request, usages, disabled_quotas, tenant_id):
    if tenant_id:
        ...
        '''
        if is_m1_user_admin(request):
            instances, has_more = server_list_nova(request, search_opts={'tenant_id': tenant_id}, all_tenants=all_tenants)
        else:
            instances, has_more = nova.server_list(request, search_opts={'tenant_id': tenant_id}, all_tenants=all_tenants)
