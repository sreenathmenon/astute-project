        ...
        try:
            ...
            if data['project'] and data['role_id']:
                if is_m1_user_admin(request):
                    ...
                    roles = []
                    if api.keystone.VERSIONS.active < 3:
                        roles = ksclient.roles.roles_for_user(new_user.id, data['project'])
                    else:
                ...
                assigned = [role for role in roles if role.id == str(data['role_id'])]
                if not assigned:
                    try:
                        if is_m1_user_admin(request):
                            ksclient = get_admin_ksclient()
                            if api.keystone.VERSIONS.active < 3:
                                ksclient.roles.add_user_role(new_user.id, data['role_id'], data['project'])
                            else:
                                ksclient.roles.grant(data['role_id'], user=new_user.id, project=data['project'], domain=domain.id)
                        else:
    ...
    def handle(self, request, data):
        ...
        try:
            if is_m1_user_admin(request):
                ...
                if api.keystone.VERSIONS.active < 3:
                    response = ksclient.users.update_password(user_id, password)
                else:
                    response = ksclient.users.update(user_id, password=password)
            else:
                response = api.keystone.user_update_password(request, user_id, password)
