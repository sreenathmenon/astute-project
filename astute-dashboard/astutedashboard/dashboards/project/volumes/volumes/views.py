...
from astutedashboard.dashboards.project.volumes \
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            for volume in _volumes:
                ...
                volume_type_size[volume.volume_type] = (volume_type_size.get(volume.volume_type) or 0) + volume.size

            for snapshot in cinder.volume_snapshot_list(self.request):
                volume_type_size[volume_types[snapshot.volume_id]] = (volume_type_size.get(volume_types.get(snapshot.volume_id)) or 0) + snapshot.size
            ...
            for volume_type, total_size in volume_type_size.iteritems():
                volume_type_usage.append({'name': volume_type, 'usage': total_size, 'quota': tenant_quotas.get(volume_type) or -1})
