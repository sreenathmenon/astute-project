...
from astutedashboard.dashboards.project.instances.workflows \
...
class SetFlavorChoiceAction(workflows.Action):
    ...
    def __init__(self, request, context, *args, **kwargs):
        super(SetFlavorChoiceAction, self).__init__(request, context, *args, **kwargs)
        #
        ...
        type_map = get_project_type_mapping(request, project_id)
        if type_map and type_map['billing_type']['code'] == 'payg':
            del self.fields['flavor']
            plans_choices = []
            print "#" * 120
            for item in get_project_plan_mappings(request, project_id, unassociated=True):
                plans_choices.append((str(item['id'])+'::'+str(item['flavor_id']), item['plan_name']))
            print plans_choices
            self.fields['plan_flavor'].choices = plans_choices
        else:
            if type_map:
                plan_maps = get_project_plan_mappings(request, project_id)
                if len(plan_maps) < 1:
...
class SetFlavorChoice(workflows.Step):
    ...
    contributes = ("old_flavor_id", "old_flavor_name", "flavors", "flavor", "plan_flavor")
...
class ResizeInstance(workflows.Workflow):
    ...
    def handle(self, request, context):
        ...
        plan = None
        plan_flavor = context.get('plan_flavor', None)
        if plan_flavor:
        ...
        try:
            ...
            if plan:
                associate_plan_mapping_with_instance(request, plan, instance_id)
