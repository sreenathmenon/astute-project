...
from astutedashboard.dashboards.project.instances \
    import utils as instance_utils
from astutedashboard.common import \
    get_plans, \
    get_project_type_mapping, \
    get_project_plan_mappings, \
    associate_plan_mapping_with_instance, \
...
class SetInstanceDetailsAction(workflows.Action):
    ...
    plan_flavor = forms.ChoiceField(label=_("Plan"),
                               help_text=_("Instance plan to launch."))
...
#            ("instance_snapshot_id", _("Boot from snapshot")),
        ...
        type_map = get_project_type_mapping(request, self.initial['project_id'])
        if type_map and type_map['billing_type']['code'] == 'payg':
            del self.fields['flavor']
            plans_choices = []
            for item in get_project_plan_mappings(request, self.initial['project_id'], unassociated=True):
                plans_choices.append((str(item['id'])+'::'+str(item['flavor_id']), item['plan_name']))
            self.fields['plan_flavor'].choices = plans_choices
            self.fields['count'].widget = forms.widgets.HiddenInput()
            self.fields['count'].value = 1
        else:
            if type_map:
                plan_maps = get_project_plan_mappings(request, self.initial['project_id'])
                if len(plan_maps) < 1:
    ...
    def get_help_text(self, extra_context=None):
        ...
        try:
            ...
            for quota in cinder.tenant_quota_get(self.request, self.request.user.tenant_id):
    ...
    def populate_image_id_choices(self, request, context):
        ...
        marks = []
        for plan_map in get_project_plan_mappings(request, context.get('project_id')):
...
class LaunchInstance(workflows.Workflow):
    ...
    def handle(self, request, context):
        ...
        plan_flavor = request.POST.get('plan_flavor')
        plan = None
        flavor = context.get('flavor')
        if plan_flavor:
            (plan, flavor) = plan_flavor.split('::')

        try:
            vm = api.nova.server_create(request,
                                   context['name'],
                                   image_id,
                                   flavor,
                                   context['keypair_id'],
                                   ...
                                   meta={'plan': str(plan)})
            # associate plan mapping with instance
            if plan:
                associate_plan_mapping_with_instance(request, plan, vm.id)
            return True
        except Exception:
            if port_profiles_supported:
